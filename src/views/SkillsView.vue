...
<h1 class="text-center py-4">Skills</h1>
  <div class="grid sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 pt-6">
    <div class="m-auto pb-4" v-for="image in images" :key="image.name">
      <img :src="image.url" :alt="image.name" style="max-width: 120px">
      <div class="grid justify-items-center">{{image.name}}</div>
    </div>
  </div>
</div>
</template>